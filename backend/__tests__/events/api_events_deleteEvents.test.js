...
describe("DELETE /api/events/delete-events", () => {
  ...
  test("should return 400 and error code when eventIds are missing", async () => {
    const { body } = await request(app).delete("/api/events/delete-events").send({}).expect(400);
